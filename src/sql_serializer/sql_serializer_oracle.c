...
serializeSampleClause(StringInfo from, SampleClauseOperator* s, int* curFromItem, List** fromAttrs)
{
    ...
    if(HAS_STRING_PROP(s,PROP_SUMMARIZATION_SAMP))
    {
    	List* attrNames = getAttrNames(((QueryOperator*) s)->schema);
		*fromAttrs = appendToTailOfList(*fromAttrs, attrNames);

		// retrieve information for sampling of less distinct values
    	List *getParams = (List *) GET_STRING_PROP(s,PROP_SUMMARIZATION_SAMP);
    	char *attrName = STRING_VALUE(getHeadOfListP(getParams));
    	char *attrAs = CONCAT_STRINGS(attrName,"cnt");
    	int sampleSize = atoi(STRING_VALUE(getNthOfListP(getParams,1)));
    	char *cond = STRING_VALUE(getTailOfListP(getParams));

//    	StringInfo newFrom = makeStringInfo();
    	...
    	if(cond != NULL)
    	{
        	appendStringInfo(from,
        							"((SELECT %s FROM "
        							" (SELECT r.%s "
        						   	" FROM (SELECT ROWNUM N FROM DUAL CONNECT BY LEVEL <= "
        						    " (SELECT ROUND(%u / count(distinct %s) + 1) AS %s FROM %s WHERE %s%s)) l, "
        						    " (SELECT distinct %s FROM %s) r)) F%u)",
    								attrName, attrName, sampleSize, attrName, attrAs, t->tableName, attrName, cond,
    								attrName, t->tableName, (*curFromItem)++);
    	}
    	else
    	{
        	appendStringInfo(from,
        							...
        							" (SELECT r.%s "
        						   	" FROM (SELECT ROWNUM N FROM DUAL CONNECT BY LEVEL <= "
        						    " (SELECT ROUND(%u / count(distinct %s) + 1) AS %s FROM %s)) l, "
        						    " (SELECT distinct %s FROM %s) r)) F%u)",
    								attrName, attrName, sampleSize, attrName, attrAs, t->tableName,
    								attrName, t->tableName, (*curFromItem)++);
    	}
    }
    else
