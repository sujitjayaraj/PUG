...
createAPIStub (void)
{
	...
	api->serializeSampleClause = NULL;
    ...
    if (!(LIST_LENGTH(q->parents) > 1 || HAS_STRING_PROP(q, PROP_MATERIALIZE)) || q == fromRoot)
    {
        switch(q->type)
        {
			...
			case T_SampleClauseOperator:
			{
				...
				api->serializeSampleClause(from, s, curFromItem, fromAttrs, api);
			}
			break;
            default:
            {
							 ...
							 SerializeClausesAPI *api) //TODO check since copied from Oracle
{
    ...
    //TODO: temp change for postgres
    if(isSubstr(ordExpr,"DBMS_RANDOM.RANDOM"))
    	ordExpr = replaceSubstr(ordExpr,"DBMS_RANDOM.RANDOM","RANDOM()");
