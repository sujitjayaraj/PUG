...
static void serializeSampleClause(StringInfo from, SampleClauseOperator* s, int* curFromItem,
...
createAPI (void)
{
    if (api == NULL)
    {
        ...
        api->serializeSampleClause = serializeSampleClause;
    }
}
...
static void
serializeSampleClause(StringInfo from, SampleClauseOperator* s, int* curFromItem, List** fromAttrs, SerializeClausesAPI *api)
{
	char* samp = NULL;
	if (s->sampPerc)
		samp = CONCAT_STRINGS(" TABLE", s->op.schema->name, " BERNOULLI(", exprToSQL(s->sampPerc), ")");
    ...
    *fromAttrs = appendToTailOfList(*fromAttrs, attrNames);
    ...
    appendStringInfo(from, "(%s%s F%u)",
    		quoteIdentifierPostgres(tableName), samp ? samp : "",
            (*curFromItem)++);
}
