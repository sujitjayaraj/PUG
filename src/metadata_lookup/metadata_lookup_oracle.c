...
assembleOracleMetadataLookupPlugin (void)
{
    ...
    plugin->getNumofDistinctValues = oracleGetNumofDistinctValues;
...
int
oracleGetNumofDistinctValues(char *attrName, char *tableName, char *cond)
{
    int numOfRows = 0;
    ...
    if(cond != NULL)
    	appendStringInfo(statement, "SELECT count(distinct %s "
    								") FROM %s "
									"WHERE %s %s ",
    	                            attrName, tableName, attrName, cond);
    else
    	appendStringInfo(statement, "SELECT count(distinct %s ) FROM %s ",
    								 attrName, tableName);
