...
//TODO: After version 11
#define NAME_IS_WIN_FUNC_12 "GPRoM_IsWinFunc"
#define PARAMS_IS_WIN_FUNC_12 1
#define QUERY_IS_WIN_FUNC_12 "SELECT bool_or(prokind = 'w') is_win FROM pg_proc " \
        "WHERE proname = $1::text;"

#define NAME_IS_AGG_FUNC_12 "GPRoM_IsAggFunc"
#define PARAMS_IS_AGG_FUNC_12 1
#define QUERY_IS_AGG_FUNC_12 "SELECT bool_or(prokind = 'a') AS is_agg FROM pg_proc " \
        "WHERE proname = $1::text;"
...
#define NAME_GET_ROWNUM "GPRoM_GetRownum"
#define PARAMS_GET_ROWNUM 1
#define QUERY_GET_ROWNUM "SELECT reltuples " \
						 "FROM pg_class " \
						 "WHERE relkind = 'r' AND relname NOT LIKE 'pg_%' AND relname NOT LIKE 'sql%' AND relname = $1::text;"

#define NAME_GET_DISTINCT "GPRoM_GetDistinct"
#define PARAMS_GET_DISTINCT 3
#define QUERY_GET_DISTINCT "SELECT count(distinct $1::text) " \
						 "FROM $2::text " \
						 "WHERE $1::text $3::text;"
...
assemblePostgresMetadataLookupPlugin (void)
{
    ...
    p->getNumofRowsInformation = postgresGetNumofRowsInformation;
    p->getNumofDistinctValues = postgresGetNumofDistinctValues;
...
int
postgresGetNumofRowsInformation(char *tableName)
{
	PGresult *res = NULL;
//	PGconn *c = plugin->conn;
	...
	// do query
	ACQUIRE_MEM_CONTEXT(memContext);
	res = execPrepared(NAME_GET_ROWNUM, singleton(createConstString(tableName)));

	if (res == NULL)
		FATAL_LOG("Can't count the number of rows failed for %s", tableName);
...
int
postgresGetNumofDistinctValues(char *attrName, char *tableName, char *cond)
{
	PGresult *res = NULL;
//	PGconn *c = plugin->conn;
	...
	// do query
	ACQUIRE_MEM_CONTEXT(memContext);
	res = execPrepared(NAME_GET_DISTINCT, CONCAT_LISTS(singleton(createConstString(attrName)),
			singleton(createConstString(tableName)), singleton(createConstString(cond))));

	if (res == NULL)
		FATAL_LOG("Can't count the number of distinct values failed for %s", tableName);
