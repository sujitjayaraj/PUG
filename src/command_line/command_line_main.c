...
#define HELLO_MESSAGE "Welcome to the PUG " PACKAGE_VERSION " command line interface"
...
//        execute(result);
    ...
    else
    {
        ...
        printf("Please input a Datalog command, '\\q' to exit the program, or '\\h' for help\n");
        printf("======================================================================\n\n");
        inputLoop();
    }
...
utility(char *command)
{
    ...
    {
        printf(TB_FG_BG(WHITE,BLACK,"PUG CLI COMMANDS") "%s",commandHelp);
        printf("\n");
        return;
    }
...
readConf (void)
{
    TRY
    {
        ...
        {
            gpromconf = CONCAT_STRINGS(userhomedir,"/.pug");
        ...
        {
            gpromhist = CONCAT_STRINGS(userhomedir,"/.pug_hist");
        }
        if(gpromhist != NULL) {
