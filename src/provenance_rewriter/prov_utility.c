...
/*
 * Find cross-product operators
 */
boolean
findCrossProductVisitor (Node *node, List **result)
{
    if (node == NULL)
        return TRUE;

    JoinOperator *cp = (JoinOperator *) node;

    if (isA(node, JoinOperator) && cp->joinType == JOIN_CROSS)
        *result = appendToTailOfList(*result, node);

    return visit(node, findCrossProductVisitor, (void *) result);
}
